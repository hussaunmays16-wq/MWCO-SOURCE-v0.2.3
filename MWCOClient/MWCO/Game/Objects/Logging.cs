...
{
	internal class Logging : ISyncedObject
	{
		public Logging(GameObject go)
		{
		...
		private void HookEvents(GameObject go)
		{
			...
			EventHook.AddWithSync(playmakerScriptByName, "Open door", delegate
			{
				go.GetComponent<ObjectSyncComponent>().TakeSyncControl();
				return false;
			}, false);
			EventHook.AddWithSync(playmakerScriptByName, "Close door", delegate
			{
				go.GetComponent<ObjectSyncComponent>().TakeSyncControl();
				return false;
			}, false);
		...
		public bool CanSync()
		{
			if ((double)(this.lastRotation - this.gameObject.transform.localRotation.x) > 0.005 || (double)(this.lastRotation - this.gameObject.transform.localRotation.x) < -0.005)
			{
				this.lastRotation = this.gameObject.transform.localRotation.x;
				return true;
			}
